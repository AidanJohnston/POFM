 ...
 * For Gabe:
 *
 * The purpose of the move header is to given an input and an output, move a file.
 *
...
bool move(ARGS arguments) {
    ...
    if(doesFileExist(arguments.input_file)) {
        if(arguments.force) {
            ...
            if(arguments.output) {
                if(doesFileExist(arguments.output_file)) {
                    printf("Are you sure you want to move %s?  %s already exist. (y/n)\n", arguments.input_file, arguments.output_file);
                    scanf("%c", &option);
                    ...
                    if(option == 121) {
                        ...
                        if(arguments.verbose)
                            printf("Moved the file\n");
                    }
                    else if(arguments.verbose) {
                        printf("Wont move the file\n");
                    }
                }
                else {
                    FILE* input = fopen(arguments.input_file, "r");
        ...
        printf("The file %s doesn't exist, could not move the file.\n", arguments.input_file);
    }
}